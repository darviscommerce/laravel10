    ...
    <ul class="mb-4 nav nav-tabs">
        <li class="nav-item">
            <a class="nav-link {{ !isset($plugin) && config('manta-cms.locale') == $locale ? 'active' : null }}"
                aria-current="page"
                href="{{ route('manta.blog.update', ['input' => $item->translation()['org']->id]) }}">{{ config('manta-cms.locales')[config('manta-cms.locale')]['language'] }}
        ...
        <li class="nav-item">
            <a class="nav-link {{ isset($plugin) && $plugin == 'uploads' ? 'active' : null }}"
                href="{{ route('manta.blog.uploads', ['locale' => $key, 'input' => $item->id]) }}">Uploads</a>
        </li>
    </ul>
@endif
