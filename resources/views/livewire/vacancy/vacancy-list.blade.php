<div class="container" wire:init="loadTrash">
    ...
    <div class="mt-3 row">
        <div class="col-4">
            <a href="{{ route('manta.vacancies.create') }}" class="btn btn-sm btn-success"><i
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @elseif ($deleteId == null || $deleteId != $item->id)
                            <a href="{{ route('manta.vacancies.update', ['input' => $item->id]) }}"
                            ...
                            @foreach (config('manta-cms.locales') as $key => $value)
                                @if ($key != config('manta-cms.locale'))
                                    @php
                                        $lang = App\Models\MantaVacancy::where(['locale' => $key, 'pid' => $item->id])->first();
                                    @endphp
                                    <a class="btn btn-sm {{ $lang ? 'btn-warning' : 'btn-success' }}"
                                        href="{{ route('manta.vacancies.update', ['locale' => $key, 'input' => $item->id]) }}"><span
